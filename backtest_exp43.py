"""
LIBERTAD_2045 — Backtest Experimento 43
=========================================
Momentum (ROC-60) como criterio de desempate en señales de pullback.

Pregunta: cuando hay más señales válidas que slots libres, ¿priorizar por
ROC-60 (Rate of Change a 60 días) en lugar del score compuesto actual
(distancia SMA50 + pendiente SMA200) mejora el resultado?

Cambio único respecto a backtest_expandido.py (baseline v3):
  - calcular_indicadores: añade columna ROC60 = close.pct_change(60)
  - Criterio de ordenación: ROC60 en lugar del score compuesto (close-SMA50)/ATR + SMA200_slope
  - Todo lo demás (señal, stop, sizing, Risk Guardian, rebalanceo) sin tocar

Uso:
    python backtest_exp43.py
...
def calcular_indicadores(df):
    ...
    # Exp43: momentum a 60 días como criterio de ordenación de señales
    df["ROC60"] = close.pct_change(60)
...
def ejecutar_backtest(datos, composicion_df=None):
    ...
    for fecha in fechas:
        ...
        for symbol in datos:
            ...
            # Exp43: ROC-60 como criterio de ordenación
            roc60 = bar.get("ROC60", np.nan)
            if pd.isna(roc60):
                roc60 = -999.0
        ...
        # Exp43: ordenar por ROC-60 en lugar del score compuesto
        señales      = sorted(señales, key=lambda x: x["roc60"], reverse=True)
...
def imprimir_informe(metricas):
    ...
    print(f"\n{sep}")
    print(f"  LIBERTAD_2045 — BACKTEST EXPERIMENTO 43")
    print(f"  {START_DATE} → {END_DATE}")
    print(f"  Momentum ROC-60 como criterio de ordenación de señales")
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("  LIBERTAD_2045 — BACKTEST EXPERIMENTO 43")
    print("=" * 50)
