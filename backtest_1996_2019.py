...
def sp500_en_fecha(comp_df: pd.DataFrame, fecha) -> set:
    ...
    Los tickers en el CSV llevan sufijo de fecha de salida (p.ej. AAL-199702).
    Se eliminan antes de devolver el set para que coincidan con los
    símbolos limpios usados internamente.
    ...
    try:
        ...
        for t in str(val).split(","):
            t = t.strip()
            if t:
                tickers.add(_date_suffix.sub('', t))
        return tickers
    except Exception:
        return None
...
def calcular_metricas(trades, curva_capital, capital_final):
    ...
    # CAGR basado en capital inicial vs capital final (ignora aportaciones anuales)
    if len(df_capital) > 1:
        fecha_ini = pd.Timestamp(df_capital["fecha"].iloc[0])
        fecha_fin = pd.Timestamp(df_capital["fecha"].iloc[-1])
        years = (fecha_fin - fecha_ini).days / 365.25
        cagr = (capital_final / CAPITAL_INICIAL) ** (1 / years) - 1 if years > 0 else 0.0
    else:
        cagr = 0.0
...
# ==================================================
# ANÁLISIS POR SUB-PERÍODOS HISTÓRICOS
# ==================================================

def calcular_subperiodos(trades, curva_capital):
    """
    Desglosa métricas por sub-período histórico relevante.
    Usa la fecha de cierre del trade para asignar el período.
    """
    PERIODOS = [
        ("1996-2000  pre-burbuja"       , "1996-01-01", "2000-03-09"),
        ("2000-2003  crash puntocom"    , "2000-03-10", "2002-10-09"),
        ("2003-2007  expansión"         , "2002-10-10", "2007-10-08"),
        ("2008-2009  crisis financiera" , "2007-10-09", "2009-03-09"),
        ("2009-2019  recuperación larga", "2009-03-10", "2019-01-11"),
    ]
    ...
    df_t["fecha_salida"] = pd.to_datetime(df_t["fecha_salida"])
    df_c   = pd.DataFrame(curva_capital)
    ...
    for nombre, inicio, fin in PERIODOS:
        ...
        t_fin = pd.Timestamp(fin)

        sub_t = df_t[(df_t["fecha_salida"] >= t_ini) & (df_t["fecha_salida"] <= t_fin)]
        sub_c = df_c[(df_c["fecha"] >= t_ini) & (df_c["fecha"] <= t_fin)]["capital"].values

        if len(sub_t) > 0:
            wins   = sub_t[(sub_t["resultado"] == "WIN") | ((sub_t["resultado"] == "OPEN→CLOSE") & (sub_t["pnl"] >= 0))]
            losses = sub_t[(sub_t["resultado"] == "LOSS") | ((sub_t["resultado"] == "OPEN→CLOSE") & (sub_t["pnl"] < 0))]
            wr = len(wins) / len(sub_t)
            gan = wins["pnl"].sum()   if len(wins)   > 0 else 0.0
            per = losses["pnl"].abs().sum() if len(losses) > 0 else 1.0
            pf  = gan / per if per > 0 else float("inf")
            pnl_periodo = sub_t["pnl"].sum()
        else:
            wr = pf = pnl_periodo = 0.0
        ...
        resultados.append({
            ...
            "profit_factor"  : pf,
            "max_drawdown"   : max_dd,
            "pnl_periodo"    : round(pnl_periodo, 2),
            "capital_inicio" : round(cap_ini, 2),
            "capital_fin"    : round(cap_fin, 2),
        })
...
def guardar_resultados(trades, curva_capital, metricas):
    ...
    if trades:
        pd.DataFrame(trades).to_csv(
            f"{LOG_DIR}/1996_2019_trades_{timestamp}.csv", index=False)
        print(f"\nTrades guardados    : {LOG_DIR}/1996_2019_trades_{timestamp}.csv")

    if curva_capital:
        pd.DataFrame(curva_capital).to_csv(
            f"{LOG_DIR}/1996_2019_capital_{timestamp}.csv", index=False)
        print(f"Curva de capital    : {LOG_DIR}/1996_2019_capital_{timestamp}.csv")

    if metricas:
        metricas_flat = {k: v for k, v in metricas.items() if k != "por_clase"}
        pd.DataFrame([metricas_flat]).to_csv(
            f"{LOG_DIR}/1996_2019_metricas_{timestamp}.csv", index=False)
        print(f"Métricas guardadas  : {LOG_DIR}/1996_2019_metricas_{timestamp}.csv")
...
def imprimir_informe(metricas, subperiodos=None):

    sep = "─" * 60

    print(f"\n{sep}")
    print(f"  LIBERTAD_2045 — BACKTEST HISTÓRICO 1996-2019")
    print(f"  {START_DATE} → {END_DATE}  (~23 años)")
    print(f"  Puntocom · Crisis 2008 · Recuperación larga")
    ...
    print(f"  Universo          : dinámico vía sp500_composicion.csv")
    print(f"                      (survivorship bias eliminado)")
    print(sep)
    ...
    print(f"  CAGR (aprox.)    : {metricas['cagr']:>12.1%}  *sin ajustar por aportaciones anuales*")
    ...
    if subperiodos:
        print(f"\n  DESGLOSE POR SUB-PERÍODO")
        print(f"  {'Período':<35} {'Trades':>6} {'WR':>6} {'PF':>6} {'MaxDD':>7} {'PnL €':>10} {'Cap.fin €':>10}")
        print(f"  {'-'*35} {'-'*6} {'-'*6} {'-'*6} {'-'*7} {'-'*10} {'-'*10}")
        for sp in subperiodos:
            pf_str = f"{sp['profit_factor']:.2f}" if sp['profit_factor'] != float("inf") else "  inf"
            print(
                f"  {sp['periodo']:<35} "
                f"{sp['n_trades']:>6d} "
                f"{sp['win_rate']:>5.1%} "
                f"{pf_str:>6} "
                f"{sp['max_drawdown']:>6.1%} "
                f"{sp['pnl_periodo']:>10.0f} "
                f"{sp['capital_fin']:>10.0f}"
            )
    ...
    print(f"\n  {'✓ ROBUSTO: sobrevive puntocom + 2008 + expansiones' if apto else '✗ SISTEMA REQUIERE AJUSTES'}")
    print(f"\n{sep}\n")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  LIBERTAD_2045 — BACKTEST HISTÓRICO 1996-2019")
    print("  Puntocom · Crisis 2008 · Recuperación larga")
    print("=" * 60)
    print(f"  Período      : {START_DATE} → {END_DATE}  (~23 años)")
    ...
    subperiodos  = calcular_subperiodos(trades, curva_capital)
    imprimir_informe(metricas, subperiodos)
