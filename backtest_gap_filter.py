...
# --------------------------------------------------
# Filtro de gaps — Experimento 36
# Si el precio de apertura supera el BUY STOP en más de
# GAP_MAX × ATR, la orden se cancela para evitar entradas
# a precios que distorsionen el sizing calculado.
# --------------------------------------------------
GAP_MAX_ATR = float(os.getenv("GAP_MAX_ATR", "1.0"))  # Umbral en múltiplos de ATR
...
def ejecutar_backtest(datos, composicion_df=None):
        ...
        if idx + 1 < len(fechas):
            ...
            for señal in señales:
                ...
                if bar_entrada["High"] >= buy_stop:
                    ...
                    # --------------------------------------------------
                    # Experimento 36 — Detección de gap de apertura
                    # Si el Open supera el BUY STOP hubo gap.
                    # Gap > GAP_MAX_ATR × ATR → cancelar orden.
                    # Gap tolerable → entrar al Open con stop recalculado.
                    # --------------------------------------------------

                    precio_open    = bar_entrada["Open"]
                    atr_señal      = señal["atr"]

                    if precio_open > buy_stop and atr_señal > 0:
                        gap = precio_open - buy_stop
                        if gap > GAP_MAX_ATR * atr_señal:
                            gaps_cancelados += 1
                            continue
                        precio_entrada = precio_open
                    else:
                        precio_entrada = buy_stop

                    stop_loss_real = round(precio_entrada - señal["stop_distance"], 4)
    ...
    print(f"    Gaps cancelados              : {gaps_cancelados}")
...
if __name__ == "__main__":
    ...
    print(f"  Gap máx. ATR : {GAP_MAX_ATR}×ATR")
