...
def detectar_senal(df, i):
    ...
    # Mejora 2+5: ventana 3 días + umbral ATR-adaptativo
    # Se busca que en alguno de los 3 días anteriores el cierre haya caído
    # al menos 0.75 × ATR por debajo de la SMA50 (umbral proporcional a la volatilidad).
    pullback = False
    for k in range(1, 4):  # k=1,2,3 → días i-1, i-2, i-3
        row = df.iloc[i - k]
    ...
    for fecha in fechas:
        ...
        for symbol, pos in posiciones.items():
            ...
            if not pd.isna(atr) and atr > 0:
                ...
                # Mejora 4: break-even — si el precio supera entry + 1.5 × ATR,
                # subir el stop a entry + 0.5 × ATR para proteger beneficios.
        ...
        for symbol in datos:
            ...
            bar          = df.iloc[i]
            # Mejora 3: score compuesto — bounce sobre SMA50 + pendiente SMA200 (5 días)
            _sma200_5d    = df.iloc[i - 5]["SMA200"] if i >= 5 else np.nan
            _sma200_slope = (
