...
# --------------------------------------------------
# Penalización por correlación — Experimento 35
# Penaliza señales muy correlacionadas con posiciones abiertas
# restando al score: score_final = score - PESO × correlacion_media
# --------------------------------------------------
CORRELACION_VENTANA = 90    # días para calcular correlación
CORRELACION_PESO    = 0.5   # peso de la penalización
...
# ==================================================
# CORRELACIÓN CON PORTFOLIO
# ==================================================

def calcular_correlacion_media(symbol, posiciones, datos, fecha, ventana):
    """
    Calcula la correlación media de un activo candidato
    con todas las posiciones actualmente abiertas.

    Usa los últimos `ventana` cierres anteriores a `fecha`.

    Retorna un float entre -1 y 1, o 0 si no hay posiciones
    abiertas o no hay datos suficientes.
    """
    if not posiciones:
        return 0.0

    df_cand = datos.get(symbol)
    if df_cand is None:
        return 0.0

    # Cierres del candidato hasta la fecha
    cierres_cand = df_cand.loc[df_cand.index <= fecha, "Close"].tail(ventana)

    if len(cierres_cand) < ventana // 2:
        return 0.0

    correlaciones = []

    for sym_pos in posiciones:
        df_pos = datos.get(sym_pos)
        if df_pos is None:
            continue

        cierres_pos = df_pos.loc[df_pos.index <= fecha, "Close"].tail(ventana)

        if len(cierres_pos) < ventana // 2:
            continue

        # Alinear por índice
        combined = pd.concat([cierres_cand, cierres_pos], axis=1).dropna()

        if len(combined) < ventana // 2:
            continue
    ...
    return sum(correlaciones) / len(correlaciones)
...
def ejecutar_backtest(datos, composicion_df=None):
    ...
    for fecha in fechas:
        ...
        for symbol in datos:
            ...
            # Experimento 35 — penalización por correlación
            # Resta al score la correlación media con posiciones abiertas
            # ponderada por CORRELACION_PESO. Activos poco correlacionados
            # suben en el ranking; activos muy correlacionados bajan.
            corr_media = calcular_correlacion_media(
                symbol, posiciones, datos, fecha, CORRELACION_VENTANA
            )
            score = score - CORRELACION_PESO * corr_media
...
if __name__ == "__main__":
    ...
    print(f"  Corr. ventana: {CORRELACION_VENTANA} días | peso: {CORRELACION_PESO}")
