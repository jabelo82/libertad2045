"""
LIBERTAD_2045 — Backtest Experimento 42
========================================
Universo combinado S&P500 + diferencial Nasdaq-100 validado.

Idéntico al Experimento 30 (baseline v3) en todo excepto el universo:
añade 4 tickers del diferencial NDX-100 con historia ≥200 días pre-2006
que NO estaban en el universo original:
  ASML (ASML Holding, listado NASDAQ 1995)
  CCEP (Coca-Cola European Partners, datos desde 2005)
  INSM (Insmed, IPO 2000)
  MSTR (MicroStrategy, IPO 1998)

MRVL ya estaba en el universo base. ALNY excluida (sin caché 2006-2025).
...
# Exp42: añadir tickers del diferencial NDX-100 validados (pre-2006 ≥200 barras)
# MRVL ya está incluido arriba. ALNY excluida por falta de caché 2006-2025.
_EXP42_NUEVOS = ["ASML", "CCEP", "INSM", "MSTR"]
SP500 = SP500 + _EXP42_NUEVOS
...
def guardar_resultados(trades, curva_capital, metricas):
    ...
    if trades:
        pd.DataFrame(trades).to_csv(
            f"{LOG_DIR}/exp42_trades_{timestamp}.csv", index=False)
        print(f"\nTrades guardados    : {LOG_DIR}/exp42_trades_{timestamp}.csv")

    if curva_capital:
        pd.DataFrame(curva_capital).to_csv(
            f"{LOG_DIR}/exp42_capital_{timestamp}.csv", index=False)
        print(f"Curva de capital    : {LOG_DIR}/exp42_capital_{timestamp}.csv")

    if metricas:
        metricas_flat = {k: v for k, v in metricas.items() if k != "por_clase"}
        pd.DataFrame([metricas_flat]).to_csv(
            f"{LOG_DIR}/exp42_metricas_{timestamp}.csv", index=False)
        print(f"Métricas guardadas  : {LOG_DIR}/exp42_metricas_{timestamp}.csv")
...
def imprimir_informe(metricas):
    ...
    print(f"  Experimento 42 — Universo combinado SP500 + NDX diferencial")
