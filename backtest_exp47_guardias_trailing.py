"""
LIBERTAD_2045 — Experimento 47: guardias anti-outlier del trailing stop
==========================================================================
Copia de trabajo de backtest_expandido.py (backtest canónico de 21 años,
PF 2,56-2,6, WR 54%). Objetivo: validar con el backtest completo (2006-2025,
el mismo periodo del backtest canónico) las guardias ATR_SALTO y
STOP_DEMASIADO_CERCA de position_size.py, añadidas el 05-07/08/2026 tras el
incidente ANET DIRECTAMENTE EN PRODUCCIÓN, sin pasar nunca por este backtest.

Tres variantes de la actualización diaria del trailing stop (idéntico al
resto del motor, solo cambia el bloque de la sección 4 "Gestionar posiciones
abiertas"):

  VARIANTE A - SIN GUARDIAS   : trailing stop original (el que valida el
                                 backtest canónico actual, referencia).
  VARIANTE B - BLOQUEO BINARIO: tal como está en producción ahora mismo —
                                 si el ratio ATR sale de
                                 [ATR_RATIO_MIN, ATR_RATIO_MAX], descarta el
                                 ciclo entero (mantiene el stop anterior).
  VARIANTE C - AMORTIGUADA    : capea (clamp) el ATR usado en el cálculo a
                                 los límites del ratio permitido en vez de
                                 descartar el ciclo, y STOP_DEMASIADO_CERCA
                                 sigue actuando como red de seguridad final
                                 sobre el resultado ya amortiguado.

LIMITACIÓN EXPLÍCITA DE ESTA COMPARACIÓN: el motor de backtest (igual que
backtest_expandido.py) NO simula gaps — la salida es siempre por Close vs
stop y se rellena al precio teórico del stop (ver
[[gap-risk-backtest-expandido]], verificado 19/08/2026). Las guardias B y C
existen precisamente para el caso de datos anómalos / eventos extremos
intradía que este motor no reproduce, así que el resultado agregado de este
experimento puede SUBESTIMAR el valor real de las guardias en producción.
No se ha añadido ninguna simulación de gap — no existe en el motor real.

Experimento 30 original: simulación fiel con Risk Guardian completo.
...
# --------------------------------------------------
# Guardias anti-outlier del trailing stop — idénticas a las constantes
# reales de position_size.py (incidente ANET, 05-07/08/2026)
# --------------------------------------------------
...
VARIANTE_TRAILING = "A"  # "A" sin guardias | "B" bloqueo binario | "C" amortiguada
...
# ==================================================
# TRAILING STOP — tres variantes de guardias anti-outlier
# ==================================================

def calcular_nuevo_stop_trailing(df, i_actual, stop_actual, variante,
                                  symbol=None, fecha=None, activaciones=None,
                                  fecha_entrada=None):
    """
    Calcula el candidato a nuevo trailing stop para el ciclo de hoy, aplicando
    las guardias anti-outlier según la variante pedida. Replica exactamente
    la lógica real de position_size.py::calcular_trailing_stop(), adaptada a
    las estructuras de datos de este backtest (columnas 'High'/'Close'/'ATR').

    No decide si el stop sube o baja respecto a stop_actual — igual que en
    producción, esa comparación ("solo sube") la hace el llamador.

    Retorna (nuevo_stop_candidato o None, atr_usado).
    None significa "ciclo descartado por una guardia" — el llamador debe
    conservar stop_actual sin modificarlo, igual que en producción.

    Cada activación de guardia se añade a `activaciones` (si se pasa una
    lista) como dict con symbol, fecha, tipo y los valores relevantes, para
    poder cruzar después con el resultado final de la operación.
    """
    bar       = df.iloc[i_actual]
    atr_val   = bar["ATR"]
    high_hoy  = bar["High"]
    close_hoy = bar["Close"]
    ...
    ratio    = None
    if i_actual >= 1:
        prev = df.iloc[i_actual - 1]["ATR"]
        if not pd.isna(prev) and prev > 0:
            atr_prev = float(prev)
            ratio    = float(atr_val) / atr_prev

    atr_usado = float(atr_val)

    # ── Guardia 1: salto extremo de ATR entre ciclos consecutivos ──
    if variante in ("B", "C") and ratio is not None:
        salto = ratio < ATR_RATIO_MIN or ratio > ATR_RATIO_MAX
        if salto:
            if activaciones is not None:
                activaciones.append({
                    "symbol": symbol, "fecha": fecha, "tipo": "ATR_SALTO",
                    "variante": variante, "atr_prev": atr_prev,
                    "atr_hoy": float(atr_val), "ratio": ratio,
                    "fecha_entrada": fecha_entrada,
                })
            if variante == "B":
                # Bloqueo binario — se descarta el ciclo entero
                return None, None
            else:
                # Variante C — se amortigua: clamp del ATR usado en el
                # cálculo a los límites del ratio permitido, y se sigue
                if ratio < ATR_RATIO_MIN:
                    atr_usado = atr_prev * ATR_RATIO_MIN
                else:
                    atr_usado = atr_prev * ATR_RATIO_MAX

    mult       = obtener_multiplicador(df, i_actual)
    nuevo_stop = round(high_hoy - atr_usado * mult * TRAILING_FACTOR, 2)

    # ── Guardia 2: el stop no puede quedar pegado al precio ──
    if variante in ("B", "C") and not pd.isna(close_hoy):
        distancia = float(close_hoy) - nuevo_stop
        if distancia < MIN_STOP_DISTANCE_ATR * atr_usado:
            if activaciones is not None:
                activaciones.append({
                    "symbol": symbol, "fecha": fecha,
                    "tipo": "STOP_DEMASIADO_CERCA", "variante": variante,
                    "stop_candidato": nuevo_stop, "close_hoy": float(close_hoy),
                    "distancia": distancia, "atr_usado": atr_usado,
                    "fecha_entrada": fecha_entrada,
                })
            # Red de seguridad final — descarta el ciclo también en C
            return None, None

    return nuevo_stop, atr_usado
...
def ejecutar_backtest(datos, composicion_df=None, variante=None):
    """
    ...
    Parámetros:
        ...
        variante       : "A"/"B"/"C" — variante de guardias del trailing stop
                         (por defecto VARIANTE_TRAILING del módulo).
    """
    ...
    if variante is None:
        variante = VARIANTE_TRAILING

    activaciones = []  # guardias disparadas este run — se cruzan con trades al final

    print("\nEjecutando backtest...\n")
    print(f"  Variante trailing : {variante}")
    ...
    for fecha in fechas:
        ...
        for symbol, pos in posiciones.items():
            ...
            if not pd.isna(atr) and atr > 0:
                nuevo_stop, _atr_usado = calcular_nuevo_stop_trailing(
                    df, i_actual, pos["stop"], variante,
                    symbol=symbol, fecha=fecha, activaciones=activaciones,
                    fecha_entrada=pos["fecha_entrada"],
                )
                if nuevo_stop is not None and nuevo_stop > pos["stop"]:
                    pos["stop"] = nuevo_stop
    ...
    n_atr_salto  = sum(1 for a in activaciones if a["tipo"] == "ATR_SALTO")
    n_stop_cerca = sum(1 for a in activaciones if a["tipo"] == "STOP_DEMASIADO_CERCA")
    print(f"\n  Guardias trailing stop (variante {variante}):")
    print(f"    ATR_SALTO             : {n_atr_salto} activaciones")
    print(f"    STOP_DEMASIADO_CERCA  : {n_stop_cerca} activaciones")

    return trades, curva_capital, capital, activaciones


def cruzar_activaciones_con_trades(activaciones, trades):
    """
    Para cada activación de guardia, busca el trade (mismo symbol +
    fecha_entrada) al que pertenece y anota su resultado final (WIN/LOSS/
    OPEN→CLOSE y pnl). Permite ver, operación por operación, si la guardia
    ayudó o perjudicó en cada caso concreto, no solo en el agregado.

    Retorna una lista de dicts (una fila por activación, enriquecida) y un
    resumen agregado por tipo de guardia × resultado del trade.
    """
    trades_por_key = {}
    for t in trades:
        trades_por_key[(t["symbol"], t["fecha_entrada"])] = t

    enriquecidas = []
    for a in activaciones:
        key   = (a["symbol"], a.get("fecha_entrada"))
        trade = trades_por_key.get(key)
        fila  = dict(a)
        if trade is not None:
            fila["resultado_trade"] = trade["resultado"]
            fila["pnl_trade"]       = trade["pnl"]
            fila["fecha_salida"]    = trade["fecha_salida"]
        else:
            # Posición todavía abierta al terminar el backtest sin llegar a
            # cerrarse via stop/OPEN→CLOSE registrado, o no localizable
            fila["resultado_trade"] = "SIN_TRADE_ASOCIADO"
            fila["pnl_trade"]       = None
            fila["fecha_salida"]    = None
        enriquecidas.append(fila)

    resumen = {}
    for fila in enriquecidas:
        k = (fila["tipo"], fila["resultado_trade"])
        resumen[k] = resumen.get(k, 0) + 1

    return enriquecidas, resumen
...
def guardar_resultados(trades, curva_capital, metricas, variante=None,
                        activaciones_enriquecidas=None, timestamp=None):

    os.makedirs(LOG_DIR, exist_ok=True)
    if timestamp is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    sufijo = f"_exp47_{variante}" if variante else ""

    if trades:
        pd.DataFrame(trades).to_csv(
            f"{LOG_DIR}/expandido_trades{sufijo}_{timestamp}.csv", index=False)
        print(f"\nTrades guardados    : {LOG_DIR}/expandido_trades{sufijo}_{timestamp}.csv")

    if curva_capital:
        pd.DataFrame(curva_capital).to_csv(
            f"{LOG_DIR}/expandido_capital{sufijo}_{timestamp}.csv", index=False)
        print(f"Curva de capital    : {LOG_DIR}/expandido_capital{sufijo}_{timestamp}.csv")

    if metricas:
        metricas_flat = {k: v for k, v in metricas.items() if k != "por_clase"}
        pd.DataFrame([metricas_flat]).to_csv(
            f"{LOG_DIR}/expandido_metricas{sufijo}_{timestamp}.csv", index=False)
        print(f"Métricas guardadas  : {LOG_DIR}/expandido_metricas{sufijo}_{timestamp}.csv")

    if activaciones_enriquecidas:
        pd.DataFrame(activaciones_enriquecidas).to_csv(
            f"{LOG_DIR}/expandido_guardias{sufijo}_{timestamp}.csv", index=False)
        print(f"Guardias guardadas  : {LOG_DIR}/expandido_guardias{sufijo}_{timestamp}.csv")
...
def imprimir_informe(metricas, variante=None):

    sep = "─" * 50

    print(f"\n{sep}")
    print(f"  LIBERTAD_2045 — EXPERIMENTO 47 — VARIANTE {variante}")
    print(f"  {START_DATE} → {END_DATE}")
    print(f"  Guardias trailing stop (ATR_SALTO / STOP_DEMASIADO_CERCA)")
...
def imprimir_comparativa(resultados):
    """
    Tabla comparativa final de las tres variantes — agregado + guardias.
    `resultados` es un dict {variante: {"metricas":..., "resumen_guardias":...}}
    """
    sep = "═" * 78
    print(f"\n{sep}")
    print("  EXPERIMENTO 47 — COMPARATIVA VARIANTES A/B/C (guardias trailing stop)")
    print(sep)

    print(f"\n  {'':22s}{'A (sin guardias)':>18s}{'B (bloqueo)':>18s}{'C (amortiguada)':>18s}")
    filas = [
        ("Capital final (€)", "capital_final", "{:>18,.0f}"),
        ("Profit Factor",     "profit_factor", "{:>18.4f}"),
        ("Win Rate",          "win_rate",      "{:>18.1%}"),
        ("Drawdown máximo",   "max_drawdown",  "{:>18.1%}"),
        ("Total trades",      "total_trades",  "{:>18d}"),
    ]
    for etiqueta, campo, fmt in filas:
        vals = []
        for v in ("A", "B", "C"):
            m = resultados[v]["metricas"]
            vals.append(fmt.format(m[campo]) if campo in m else f"{'—':>18s}")
        print(f"  {etiqueta:22s}" + "".join(vals))

    print(f"\n  Activaciones de guardias (0 en variante A, por diseño):")
    for v in ("B", "C"):
        resumen = resultados[v]["resumen_guardias"]
        print(f"\n  Variante {v}:")
        tipos = sorted(set(k[0] for k in resumen))
        for tipo in tipos:
            entradas = {k[1]: n for k, n in resumen.items() if k[0] == tipo}
            total = sum(entradas.values())
            print(f"    {tipo:22s}: {total} activaciones totales")
            for resultado, n in sorted(entradas.items()):
                print(f"      → trade terminó en {resultado:16s}: {n}")
...
if __name__ == "__main__":
    ...
    print("=" * 78)
    print("  LIBERTAD_2045 — EXPERIMENTO 47")
    print("  Validación con backtest de 21 años de las guardias anti-outlier")
    print("  del trailing stop (ATR_SALTO / STOP_DEMASIADO_CERCA)")
    print("=" * 78)
    ...
    print(f"\n  LIMITACIÓN: este motor NO simula gaps (salida siempre por Close vs")
    print(f"  stop, rellenada al precio teórico). Las variantes B/C existen para")
    print(f"  el caso de datos anómalos/gaps que este backtest no reproduce — el")
    print(f"  agregado de abajo puede SUBESTIMAR el valor real de las guardias.")

    # --------------------------------------------------
    # Universo dinámico del S&P500 — se carga UNA sola vez y se reutiliza
    # en las tres variantes (mismos datos, mismo universo, única variable
    # que cambia es la lógica de guardias del trailing stop)
    # --------------------------------------------------
    comp_df  = cargar_composicion_sp500()
    universo = universo_historico_sp500(comp_df)

    print(f"\n  Universo     : {len(universo)} activos históricos únicos")

    # Filtrar a lo que ya está en caché local (data/) para no depender de
    # descargas en vivo de Yahoo Finance durante el experimento — esta misma
    # brecha de 351 tickers está documentada de antes en faltan_352.txt
    # (mayoritariamente tickers ya deslistados/quebrados, pre-caché), no es
    # algo introducido por este experimento. Sin este filtro, cada símbolo
    # sin caché dispara reintentos de red con backoff de hasta 30-90s cada
    # uno bajo rate limit de Yahoo — inviable para 3 variantes x ~350 fallos.
    universo_cache = [
        s for s in universo
        if os.path.exists(f"data/{s.replace('-', '_').replace(' ', '_')}_{START_DATE}_{END_DATE}.csv")
    ]
    faltan = len(universo) - len(universo_cache)
    if faltan:
        print(f"  Sin caché    : {faltan} tickers excluidos (ver faltan_352.txt — "
              f"gap preexistente, no de este experimento; sin descarga en vivo)")

    datos = descargar_datos(universo_cache, START_DATE, END_DATE)
    ...
    for variante in ("A", "B", "C"):
        ...
        trades, curva_capital, capital_final, activaciones = ejecutar_backtest(
            datos, composicion_df=comp_df, variante=variante,
        )
        metricas = calcular_metricas(trades, curva_capital, capital_final)
        imprimir_informe(metricas, variante=variante)

        activ_enriq, resumen_guardias = cruzar_activaciones_con_trades(activaciones, trades)

        guardar_resultados(
            trades, curva_capital, metricas, variante=variante,
            activaciones_enriquecidas=activ_enriq, timestamp=timestamp_run,
        )

        resultados[variante] = {
            "metricas": metricas,
            "resumen_guardias": resumen_guardias,
            "n_activaciones": len(activaciones),
        }
