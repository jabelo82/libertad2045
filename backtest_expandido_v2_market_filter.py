...
# --------------------------------------------------
# Filtro de mercado global — Experimento 33
# Si SPY cierra bajo SMA200 → no se abren nuevas posiciones
# --------------------------------------------------
MARKET_FILTER_ACTIVO = True
MARKET_PROXY         = "SPY"
...
# ==================================================
# FILTRO DE MERCADO — SPY
# ==================================================

def descargar_spy(start, end):
    """
    Descarga datos históricos del SPY y calcula SMA50 y SMA200.
    Devuelve un DataFrame indexado por fecha, o None si falla.
    """
    try:
        df = yf.Ticker(MARKET_PROXY).history(start=start, end=end, auto_adjust=True)
        if df is None or len(df) < 200:
            print(f"  [WARN] SPY: datos insuficientes ({len(df) if df is not None else 0} barras)")
            return None
        df.index = pd.to_datetime(df.index).tz_localize(None)
        df["SMA50"]  = df["Close"].rolling(50).mean()
        df["SMA200"] = df["Close"].rolling(200).mean()
        print(f"  SPY descargado: {len(df)} barras ({df.index[0].date()} → {df.index[-1].date()})")
        return df
    except Exception as e:
        print(f"  [ERROR] No se pudo descargar SPY: {e}")
        return None


def mercado_permite_entradas_bt(spy_df, fecha):
    """
    Evalúa el estado del mercado en una fecha concreta del backtest.
    Devuelve True si el mercado permite nuevas entradas.
    Devuelve True si el filtro está desactivado o SPY no disponible.
    """
    if not MARKET_FILTER_ACTIVO:
        return True

    if spy_df is None:
        return True  # Sin datos → no bloquear

    # Buscar la barra más reciente disponible en o antes de la fecha
    fechas_spy = spy_df.index[spy_df.index <= fecha]
    if len(fechas_spy) == 0:
        return True

    ultima = spy_df.loc[fechas_spy[-1]]

    close  = ultima["Close"]
    sma200 = ultima["SMA200"]

    if pd.isna(sma200):
        return True  # SMA200 no calculable todavía → no bloquear

    return close > sma200  # False = BAJISTA → bloquear entradas
...
def ejecutar_backtest(datos, composicion_df=None, spy_df=None):
    ...
    Parámetros:
        ...
        spy_df         : DataFrame con datos SPY para filtro de mercado
    """
    ...
    for fecha in fechas:
        ...
        # --------------------------------------------------
        # 7b. Filtro de mercado global — Experimento 33
        # Si SPY cierra bajo SMA200 no se abren nuevas posiciones.
        # Las posiciones existentes mantienen sus stops activos.
        # --------------------------------------------------
        if not mercado_permite_entradas_bt(spy_df, fecha):
            dias_bloqueados_mercado += 1
            curva_capital.append({"fecha": fecha, "capital": round(capital, 2)})
            continue
    ...
    print(f"    Días bloqueados por mercado  : {dias_bloqueados_mercado}")
...
if __name__ == "__main__":
    ...
    # Descargar SPY para el filtro de mercado
    spy_df = None
    if MARKET_FILTER_ACTIVO:
        print("\nDescargando datos del mercado (SPY)...")
        spy_df = descargar_spy(START_DATE, END_DATE)
        if spy_df is None:
            print("  [WARN] SPY no disponible — filtro de mercado desactivado para este backtest")

    trades, curva_capital, capital_final = ejecutar_backtest(datos, composicion_df=comp_df, spy_df=spy_df)
