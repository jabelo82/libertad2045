...
**Actualización 2026-07-20 — causa raíz cuantificada, no solo mitigada:** El fix del 20/06 evita que el servicio se quede caído, pero no elimina la causa del choque de sesión — solo lo hace invisible porque se recupera solo en ~1 minuto. Confirmado hoy con evidencia directa: journalctl muestra un patron de reinicios "extra" (ademas del reinicio diario esperado a las 01:45) repitiendose casi a diario desde el 04/07, en horarios de tarde-noche variables. Contrastado con un caso real: Javier abrio la app movil de IBKR a las 12:26 (timestamp visible en capturas de pantalla) → el journal registra el reinicio del Gateway a las 12:27:09, un minuto despues. Ademas, Javier confirma que la propia app movil tambien lo expulsa a el tras ~1 minuto de uso — es decir, el Gateway del VPS y la sesion movil se expulsan mutuamente cada vez que ambos usan las mismas credenciales (`jabelo1982`) a la vez.
Riesgo no cubierto: si esto ocurriera durante la ventana del ciclo nocturno (22:10-22:14) o mientras se gestiona un cierre de posicion, el minuto de caida podria coincidir con el momento exacto en que el bot necesita la conexion, causando un fallo de ciclo que el watchdog de las 12:00 no detectaria hasta 14h despues.
Pendiente (no urgente, pero recomendado): retomar la gestion del usuario paper separado (`jabelo1982paper`, estado "Pendiente" en el portal IBKR segun el contexto del proyecto) o un usuario de solo consulta, para que abrir la app movil deje de disputar la sesion con el bot.
Confirmacion visual directa (20/07/2026): Javier vio el mensaje "sesion existente" DENTRO de la app movil al abrirla a las 12:26 — confirma el choque de sesion sin ambiguedad, no es solo coincidencia de horarios. Aclaracion importante: no hubo aviso de verificacion en dos pasos (2FA) — ese tipo de notificacion solo aparece en logins desde navegador/PC, no aplica al Gateway automatizado ni a la app movil normal, asi que se descarta como factor en este incidente.
