"""
LIBERTAD_2045 — Backtest Experimento 44
=========================================
Universo Russell 2000 — ¿aporta valor incremental real?

Pregunta: ¿añadir small-caps (Russell 2000) mejora el resultado, o repite
la conclusión del experimento 42 (saturación de slots, sin mejora)?

Cambio único respecto a backtest_expandido.py (baseline v3):
  - Universo: S&P500 histórico + diferencial Russell 2000 validado
    (generado por exp44_universo.py → exp44_universo_combinado.py)
  - Todo lo demás (señal, stop, sizing, Risk Guardian, rebalanceo) sin tocar

El universo dinámico S&P500 (composición histórica fja05680/sp500) sigue
activo para eliminar survivorship bias en los componentes SP500 históricos.
Los tickers del diferencial Russell 2000 se escanean siempre (sin filtro
histórico de composición, ya que Russell 2000 no tiene dataset equivalente).

Uso:
    # Paso 1: generar universo combinado
    python exp44_universo.py
    # Paso 2: ejecutar backtest
    python backtest_exp44.py

Tiempo estimado con caché: 2-5 minutos (universo mayor)
"""
...
# --------------------------------------------------
# Exp44: Universo combinado S&P500 + Russell 2000
# Generado por exp44_universo.py → exp44_universo_combinado.py
# --------------------------------------------------

try:
    from exp44_universo_combinado import SP500 as _SP500_COMBINADO
    SP500 = _SP500_COMBINADO
    print(f"  Universo Exp44 : {len(SP500)} tickers (S&P500 + Russell 2000 validado)")
except ImportError:
    # Fallback: si no se ha ejecutado exp44_universo.py, usamos el universo base
    from universe_sp500 import SP500 as _SP500_BASE
    SP500 = _SP500_BASE
    print(f"  AVISO: exp44_universo_combinado.py no encontrado — usando universo SP500 base ({len(SP500)} tickers)")
    print(f"  Ejecuta primero: python exp44_universo.py")
...
def imprimir_informe(metricas):
    ...
    print(f"\n{sep}")
    print(f"  LIBERTAD_2045 — BACKTEST EXPERIMENTO 44")
    print(f"  {START_DATE} → {END_DATE}")
    print(f"  Universo S&P500 + Russell 2000 — {len(SP500)} tickers")
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("  LIBERTAD_2045 — BACKTEST EXPERIMENTO 44")
    print("=" * 50)
